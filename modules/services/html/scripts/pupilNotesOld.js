            ...
            for ( i=0; i<this.degrees.length && allFieldsValid; i++ )
            {
                ...
                if (noteField.value.length > 0 )
                {
                    ...
                    if ( this.degrees[i].isSubstitutive == 'f' )
                    {
                        ...
                        atualNote = parseFloat(noteFieldValue) * weight
                    }
                    else
                    {
                        for ( ii=0; ii<this.degrees.length && allFieldsValid && ( parseFloat(noteField.value) * weight) > 0; ii++ )
                        {
                            if ( this.degrees[ii].isSubstitutive == 'f' )
                            {
                                noteField2 = document.getElementById('degree[' + this.enrollId + '][' + this.degrees[ii].degreeId + ']')
                                weight2 = this.degrees[ii].weight
                                noteField2Value = noteField2.value == "--" ? 0 : noteField2.value;
                                if ( minimumNote == 11 )
                                {
                                    minimumNote = parseFloat(noteField2Value) * weight2
                                    minimumWeight = weight2
                                }

                                if ( minimumNote > ( parseFloat(noteField2Value) * weight2 ) )
                                {
                                    minimumNote = parseFloat(noteField2Value) * weight2
                                    minimumWeight = weight2
                                }
                        ...
                        if ( minimumNote != 11 )
                        {
                            noteSum -= minimumNote * minimumWeight
                            weightSum -= minimumWeight
                            atualWeight = ( ( parseFloat(noteField.value) * weight ) > minimumNote ) ? weight : minimumWeight;
                            atualNote = ( ( parseFloat(noteField.value) * weight ) > minimumNote ) ? ( parseFloat(noteField.value) * weight ) : minimumNote;
                        }
                        else
                        {
                            atualNote = 0
    ...
    this.updateFinalNote = function()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    var noteSum = parseFloat(averageField.value) * this.averageWeight
                    var weightSum = this.averageWeight
                }
